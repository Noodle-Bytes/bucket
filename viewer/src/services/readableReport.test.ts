...
describe("buildReportModel", () => {
    ...
    test("always populates results", () => {
        const model = buildReportModel([createNestedReadout()]);
        const readout = model.readouts[0];
        const leaf = readout.roots[0].children[0].children[0];
        expect(readout.hasResults).toBe(true);
        expect(leaf.results).toEqual({ hits: 25, hitBuckets: 3, fullBuckets: 2 });
    });

    test("drops results when the readout recorded no hits", () => {
        const model = buildReportModel([createNestedReadout({ zeroHits: true })]);
        const readout = model.readouts[0];
        const leaf = readout.roots[0].children[0].children[0];
        expect(readout.hasResults).toBe(false);
...
describe("summary rollups", () => {
    test("tier and tag tables aggregate the definition with hit columns", () => {
        const html = buildReadableReportHtml([createFlatReadout()]);
        ...
        expect(html).toContain(
            '<tr><th>Tier</th><th class="num">Coverpoints</th><th class="num">Buckets</th>' +
                ...
                '<th class="num">Hits</th><th class="num">Hit %</th></tr>',
        );
        // Tier 0 (alpha): 1 coverpoint, target 10, all 10 hits -> 100%.
        expect(html).toContain(
            '<tr><td>0</td><td class="num">1</td><td class="num">2</td>' +
                '<td class="num">2</td><td class="num">10</td><td class="num">10</td>' +
                '<td class="num"><span class="pct full">100.00%</span></td></tr>',
        );
        // uart tag (alpha + gamma): target 20, 10 hits -> 50%.
        expect(html).toContain(
...
describe("serializeReportHtml", () => {
    test("default report contains tree, cards, axes, goals, and results", () => {
    ...
    test("report includes coloured hit information", () => {
        const html = buildReadableReportHtml([createNestedReadout()]);
        expect(html).toContain(
    ...
    test("omits all results when the readout recorded no hits", () => {
        const html = buildReadableReportHtml([createNestedReadout({ zeroHits: true })]);
        // No results bar on cards, no hit badges, no rollup hit columns.
        expect(html).not.toContain('class="results"');
        expect(html).not.toContain("hits ");
        expect(html).not.toContain('<th class="num">Hits</th>');
        expect(html).not.toContain('class="pct');
        // The rest of the report is unchanged — axes, goals, buckets remain.
        expect(html).toContain('<table class="axes">');
        expect(html).toContain("Buckets: 6 total, 5 valid · Target hits: 50");
        expect(html).toContain("<h3>Summary</h3>");
    });
