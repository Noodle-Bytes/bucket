...
/**
 * Human-readable Markdown coverage report.
 *
...
export type ReadableReportOptions = {
    /** Include point descriptions. Default true. */
    description?: boolean;
    /** Include point motivations. Default true. */
    motivation?: boolean;
    /** Include tier and tags lines. Default true. */
    tierTags?: boolean;
    /** Include per-point axis tables. Default true. */
    axes?: boolean;
    /** Include the values column in axis tables. Default true. */
    axisValues?: boolean;
    /** Include per-point goal tables. Default true. */
    goals?: boolean;
    /** Include bucket counts and targets. Default true. */
    bucketCounts?: boolean;
    /** Include recorded results (hits, buckets hit/full). Default false. */
    results?: boolean;
    /** Cap on listed values per axis; 0 means unlimited. Default 64. */
    maxAxisValues?: number;
    /** Include the per-tier/per-tag rollup summary tables. Default true. */
    summary?: boolean;
...
export type RollupRow = {
    ...
    validBuckets: number;
    target: number;
    /** Total hits across the row's coverpoints; null when results are off. */
    hits: number | null;
};
...
function buildAxes(
    ...
    for (const axis of readout.iter_axes(point.axis_start, point.axis_end)) {
        ...
        if (options.axisValues) {
            ...
            values = kept.map((entry) => entry.text);
            omittedValues =
...
function buildReadoutModel(
    ...
    for (const point of readout.iter_points()) {
        ...
        const reportPoint: ReportPoint = {
            ...
            target: point.target,
            axes: !isGroup && options.axes ? buildAxes(readout, point, options) : [],
            goals: !isGroup && options.goals ? buildGoals(readout, point) : [],
            results:
                options.results && hit
                    ? {
                          hits: hit.hits,
        ...
        const children =
            point.children.length > 0
                ? // Indent guides take the same tint as the group's card.
                  ...
                  renderTreeItems(point.children, options, ids) +
                  `</ul>`
                : "";
...
function renderTree(
    ...
    return `<ul>${renderTreeItems(points, options, ids)}</ul>`;
}
...
function renderProse(point: ReportPoint, options: ResolvedReportOptions): string {
    const parts: string[] = [];
    if (options.description && point.description) {
        parts.push(`<p class="prose">${escapeHtml(point.description)}</p>`);
    }
    if (options.motivation && point.motivation) {
        parts.push(
...
function summaryStatsHtml(
    ...
    const stats: string[] = [];
    if (options.bucketCounts) {
        stats.push(`${point.bucketCount} buckets`);
    }
    if (point.results) {
        stats.push(pctBadge(point.results.hits, point.target));
    }
    return stats.length > 0
        ? `<span class="sum-stats">${stats.join(" · ")}</span>`
        : "";
}
...
function renderCardTree(
    ...
    if (!point.isGroup) {
        return renderPointCard(point, options, id);
    }
    ...
    const children = point.children
        .map((child) =>
            renderCardTree(
                ...
                options,
                ids,
                child.isGroup ? siblingGroupIdx++ % 2 : 0,
            ),
        )
        .join("");
    return (
        ...
        `${summaryStatsHtml(point, options)}</summary>` +
        `<div class="card-body">${renderProse(point, options)}${children}</div>` +
        `</details>`
    );
}
...
function renderPointCard(
    ...
    const body: string[] = [renderProse(point, options)];

    if (options.tierTags && (point.tier !== null || point.tags.length > 0)) {
        const chips: string[] = [];
        if (point.tier !== null) {
    ...
    if (options.axes && point.axes.length > 0) {
        const header = options.axisValues
            ? "<tr><th>Axis</th><th>Description</th><th class='num'>Count</th><th>Values</th></tr>"
            : "<tr><th>Axis</th><th>Description</th><th class='num'>Count</th></tr>";
            ...
            if (options.axisValues) {
                const suffix =
                    axis.omittedValues > 0
                        ...
                        : "";
                cells.push(`<td>${renderCodeList(axis.values)}${suffix}</td>`);
            }
    ...
    if (options.goals && point.goals.length > 0) {
    ...
    return (
        ...
        `<summary>${POINT_ICON}${pathHtml(point)}${summaryStatsHtml(point, options)}</summary>` +
    ...
    for (const readout of model.readouts) {
        ...
        parts.push(
            `<aside class="toc"><h3>Coverage tree</h3>` +
                `<nav class="tree">${renderTree(readout.roots, model.options, ids)}</nav>` +
                `</aside>`,
        );
        ...
        if (model.options.summary) {
            ...
            parts.push(
                renderRollupTable("Tier", readout.tierSummary, model.options.results),
            );
            if (readout.tagSummary.length > 0) {
                parts.push(
                    renderRollupTable("Tag", readout.tagSummary, model.options.results),
                );
            }
        }
