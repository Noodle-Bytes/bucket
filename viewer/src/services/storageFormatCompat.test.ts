...
type ExpectedPointHit = Record<string, unknown> & {
    waived_buckets?: number;
    waived_target?: number;
};

/**
 * Fill in fields that only exist in later formats so frozen v1/v2 snapshots
 * still compare equal to today's (richer) readback: format 3 added
 * waived_buckets / waived_target to point_hit and the bucket_waivers table.
 */
function withFormatDefaults(record: Record<string, unknown>): Record<string, unknown> {
    const pointHits = (record.point_hit as ExpectedPointHit[] | undefined) ?? [];
    return {
        ...record,
        point_hit: pointHits.map((pointHit) => ({
            ...pointHit,
            waived_buckets: pointHit.waived_buckets ?? 0,
            waived_target: pointHit.waived_target ?? 0,
        })),
        bucket_waivers: record.bucket_waivers ?? [],
    };
}
...
function fixtureDir(version: number): string {
    return join(FIXTURE_ROOT, `v${version}`);
}
...
function canonicalRecord(readout: Readout): Record<string, unknown> {
    return {
        ...
        bucket_waivers: Array.from(readout.iter_bucket_waivers(0, null)).map((waiver) => [
            waiver.start,
            waiver.reason,
        ]),
    };
    ...
    test("a frozen fixture exists for every supported format version", () => {
        const missing = SUPPORTED_VERSIONS.flatMap((version) =>
            FIXTURE_FILES.filter(
                (name) => !existsSync(join(fixtureDir(version), name)),
