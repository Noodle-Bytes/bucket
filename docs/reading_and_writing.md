<!--
  ...
  ~ Copyright (c) 2025-2026 Noodle-Bytes. All Rights Reserved
  -->
...
`PointReader` reads coverage directly from a live covertree in memory.
...
which processes a readout. Writers may for example:
  ...
  - Write the readout into another format such as SQLite, JSON, or `.bktgz`.
  - Produce an HTML report on the readout.

## Accessors

Accessors provide an API on top of readouts to access data in a hierarchical
object form. This is useful for writers, particularly those for displaying
data to humans.

| Accessor | Format | Notes |
| -- | -- | -- |
| `ArchiveAccessor` | `.bktgz` | Primary viewer format; supports multiple records per file |
